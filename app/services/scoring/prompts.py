...
def build_evaluation_prompt(
    ...
    evidence_score_result: dict | None = None,
) -> str:
    ...
    # 第一轮新增：证据信息块
    evidence_block = "未抽取到有效证据。"
    if evidence_list and evidence_score_result:
        evidence_block = f"""
# 预抽取的证据片段（已评分）
{json.dumps({
    "evidence_list": evidence_list,
    "evidence_score": evidence_score_result
...
5. evidence_quotes 优先从【预抽取的证据片段】中选择；如果没有可引用原文，返回空数组。
